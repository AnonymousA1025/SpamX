...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __ᴛʜᴜɴᴅᴇʀx__")
        end = datetime.now()
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`ʀᴇsᴛᴀʀᴛɪɴɢ ᴛʜᴜɴᴅᴇʀx...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"✗ **__ᴀᴅᴅɪɴɢ ᴜꜱᴇʀ ᴀꜱ ꜱᴜᴅᴏ ᴏɴ ᴛʜᴜɴᴅᴇʀx...__**")
        target = ""
        ...
        await event.reply("✗ ꜱᴏʀʀʏ, ᴏɴʟʏ ᴏᴡɴᴇʀ ᴏғ ᴛʜᴜɴᴅᴇʀx ᴄᴀɴ ᴀᴄᴄᴇꜱꜱ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ.")
